#!/usr/bin/env bash
# One-command dev setup: start Postgres, wait for it, build the gate binary.
set -euo pipefail
...
==> Setup complete. Run the following in your shell, then follow the steps:
    ...
    export GATE_CONFIG="$DEV/config.yaml"
