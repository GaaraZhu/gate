...
| Command | Purpose |
|---|---|
| `gate init [--harness claude-code\|opencode\|cursor\|copilot-cli\|codex\|gemini\|codebuddy] [--scope global\|project]` | Register the hook in the agent harness. `claude-code` (default) writes `~/.claude/settings.json`; `opencode` writes a TypeScript plugin at the chosen scope; `cursor` writes `~/.cursor/hooks.json` (or `.cursor/hooks.json` for project scope); `copilot-cli` writes `.github/hooks/PreToolUse.json` in the current repo root; `codex` writes `~/.codex/hooks.json` (or `.codex/hooks.json` for project scope); `gemini` writes `~/.gemini/settings.json` (or `.gemini/settings.json` for project scope); `codebuddy` writes `~/.codebuddy/settings.json` (or `.codebuddy/settings.json` for project scope). When run inside a git repo, also ensures `.gate/config.yaml` exists (blank starter if missing) and merges it into your personal config — see [docs/team.md](team.md). |
