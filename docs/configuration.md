...
For a team-shared config that's merged on top of this one, see [docs/team.md](team.md) — a git-tracked `config.yaml`, written with `gate export` and picked up with `gate config --sync`.
