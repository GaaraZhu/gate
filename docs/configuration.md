...
For a team-shared config that's merged on top of this one, see [docs/team.md](team.md) — a git-tracked `.gate/config.yaml` at the repo root, with `gate export` to write it and `gate config --sync` to pick it up.
