...
**Non-goal:** Cursor, Gemini CLI, Aider — still deferred. The Copilot CLI work in Milestone 9 stays deferred.
...
5. **Plugin file accidentally checked into a public repo (project scope).** Same risk Milestone 9 flagged for `.github/hooks/hooks.json`. Mitigation: same — README install section recommends `.gitignore`-ing `.opencode/plugin/redact.ts` if the repo is public.
...
## Milestone 9 — GitHub Copilot CLI support

> **Status: DEFERRED to a future release.** The full spec below is retained for when we revisit. Reason for deferral: Copilot CLI's `preToolUse` hook contract only supports deny-with-suggestion, not transparent rewrite, so the integration is *advisory* — strictly safer than no hook, but the AI could ignore the suggestion. We're waiting for either (a) Copilot CLI to gain an `updatedInput` equivalent, or (b) clear user demand that justifies shipping the advisory-only mode. Milestone 8 (opencode) ships first because opencode's plugin contract supports enforcing rewrite. When Copilot CLI is picked up again, renumber as needed and re-validate the schema against the current `https://docs.github.com/en/copilot/reference/hooks-configuration` (it may have drifted).
